 ...
 *     EXPR >= EXPR
 *
 * Each `EXPR` is resolved through zexpr_eval_symbols() so the
 * full address-expression vocabulary (numbers, registers,
 * map-relative names, symbols, module:symbol) is available.
 *
 * On parse/evaluation success returns 0 and stores 1/0 into
 * `*resultp`.  Returns -1 on parse failure or when an operand
 * cannot be resolved; `*resultp` is left unchanged in that case.
 *
 * No parentheses, no boolean operators, no memory dereference,
 * no side effects.
 */
int  zcond_eval(const char *s, const struct zregs *regs,
